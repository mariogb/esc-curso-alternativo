...
public class InvoiceLineOutService extends AbstractServiceLon<InvoiceLineOut> {
    ...
    /**
     * private static String sql00 = "SELECT invoice_line_out.id as
     * invoice_line_out_id, invoice_line_out.pkey as invoice_line_out_pkey,
     ...
     * invoice_line_out.orden as invoice_line_out_orden, invoice_line_out.status
     * as invoice_line_out_status, invoice_line_out.supply_date as
     * invoice_line_out_supply_date, invoice_line_out.supply_quantity as
     * invoice_line_out_supply_quantity, invoice_line_out.tax_porcent as
     * invoice_line_out_tax_porcent, invoice_line_out.total as
     * invoice_line_out_total, invoice_line_out.total_cost as
     * invoice_line_out_total_cost, invoice_line_out.unit_cost as
     * invoice_line_out_unit_cost, comercial_document.id as
     * comercial_document_id,comercial_document.pkey as
     * comercial_document_pkey,comercial_document.pname as
     * comercial_document_pname, contract.id as contract_id, contract.pkey as
     * contract_pkey,contract.pname as contract_pname,
     * program_base_time_period.id as program_base_time_period_id,
     * program_base_time_period.pkey as program_base_time_period_pkey,
     * third_person.id as third_person_id, third_person.pkey as
     * third_person_pkey,third_person.pname as third_person_pname, user_autor.id
     * as user_autor_id, user_autor.pkey as user_autor_pkey,user_autor.pname as
     ...
     * comercial_document_type_pname, product.id as product_id,product.pkey as
     * product_pkey,product.pname as product_pname, product_type.id as
     * product_type_id, product_type.pkey as
     * product_type_pkey,product_type.pname as product_type_pname,
     * stock_rack_product.id as stock_rack_product_id,stock_rack_product.pkey as
     * stock_rack_product_pkey,stock_rack_product.pname as
     * stock_rack_product_pname, stock_rack.id as stock_rack_id, stock_rack.pkey
     * as stock_rack_pkey,stock_rack.pname as stock_rack_pname, work_space.id as
     * work_space_id, work_space.pkey as work_space_pkey,work_space.pname as
     * work_space_pname FROM invoice_line_out, comercial_document_in as
     * comercial_document, contract_in as contract, program_base_time_period as
     * program_base_time_period, third_person as third_person, user_lon as
     * user_autor, comercial_document_type_in as comercial_document_type,
     * product as product, product_type as product_type, stock_rack_product as
     * stock_rack_product, stock_rack as stock_rack, work_space as work_space
     * WHERE invoice_line_out.comercial_document_id = comercial_document.id AND
     * comercial_document.contract_id = contract.id AND
     * contract.program_base_time_period_id = program_base_time_period.id AND
     * contract.third_person_id = third_person.id AND
     * comercial_document.user_autor_id = user_autor.id AND
     * comercial_document.comercial_document_type_id =
     * comercial_document_type.id AND invoice_line_out.product_id = product.id
     * AND product.product_type_id = product_type.id AND
     * invoice_line_out.stock_rack_product_id = stock_rack_product.id AND
     * stock_rack_product.stock_rack_id = stock_rack.id AND
...
//[3] workSpace --   workSpace
    ...
    private int fillXRow0(final Row r, final XSSFRow row, int nc, final boolean withIds) {
        ...
        iToCell(r, row, "invoice_line_out_orden", nc++);     //status       
