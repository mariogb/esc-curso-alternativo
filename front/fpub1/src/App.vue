<template>
  <header class="header first">
    ...
    <div v-if="isAuth" class="menu-auth-usr">
      ...
      >
         {{ elJWTParsed.username }}</mark
      >
      <a style="display:none;" href="#/docCom1"> | Comprar</a>
      <a href="#/w/simpledc/alumno"> | Alumnos</a>
      <a href="#/w/simpledc/materia"> | Materias</a>
      <a href="#/w/simpledc/calificacion"> | Calificaciones</a>
      <a href="#/w/mxm/calificacion/alumno/materia">| Alumno X Materia</a>
      
      

      <button v-on:click="chatMin = !chatMin"> Video Conferencia <small>{{ chatMin }}</small></button>
      <button class="button logout" v-if="isAuth === true" @click="doLogout()">
        [Logout]
      </button>
    </div>
    <div v-if="!isAuth">
      <button style="display:none;" v-on:click="showRegOLog = 'REGISTER'">REGISTER</button>
      <button style="display:none;"  v-on:click="showRegOLog = 'LOGIN'">LOGIN</button>
    </div>
  </header>
  <main class="main">
