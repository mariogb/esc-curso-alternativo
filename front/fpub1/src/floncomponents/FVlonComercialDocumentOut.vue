...
.dc_p_container
  .dc_p.col-0
    ...
    .resac
      .resacf1(v-if="ldco00_contract && ldco00_contract.l")
        div(v-for="(e0, i0) in ldco00_contract.l", :key="e0.id"  v-on:click="select00('contract',e0)")
          span {{ e0 }}--
      ...
      .ff
        ...
        input(
          type="text",
          v-model="item_ac_contract['pkey']",
          autocomplete="off",
          placeholder="buscar",
          name="pkey",
          @input="updateAC_contract('pkey')",
      ...
      .ff
        ...
        input(
          type="text",
          v-model="item_ac_contract['pname']",
          autocomplete="off",
          placeholder="buscar",
          name="pname",
          @input="updateAC_contract('pname')",
          v-on:keyup.enter="select01()"
        )
  ...
  .dc_p.col-1
    ...
    .resac
      .resacf1(
        v-if="ldco00_comercialDocumentType && ldco00_comercialDocumentType.l"
      )
        div(v-for="(e0, i0) in ldco00_comercialDocumentType.l", :key="e0.id", v-on:click="select00('comercialDocumentType',e0)")
          span {{ e0 }}
      ...
      .ff
        ...
        input(
          type="text",
          v-model="item_ac_comercialDocumentType['pkey']",
          autocomplete="off",
          placeholder="buscar",
          name="pkey",
          @input="updateAC_comercialDocumentType('pkey')",
          v-on:keyup.enter="select01()"
        )
      ...
      .ff
        ...
        input(
          type="text",
          v-model="item_ac_comercialDocumentType['pname']",
          autocomplete="off",
          placeholder="buscar",
          name="pname",
          @input="updateAC_comercialDocumentType('pname')",
          v-on:keyup.enter="select01()"
        )

    .cont-dcp(
      v-if="m_dc_comercialDocumentType && m_dc_comercialDocumentType.l"
    )
      .l-loop(
        v-for="e0 in m_dc_comercialDocumentType.l",
        :key="'comercialDocumentType_' + e0.id"
      ) 
        .f.pkey {{ e0.pkey }}
  ...
  .el-item(v-if="item_comercialDocumentOut")
    .el-item-parents 
      ...
      .dcp.pp-1
        ...
        .item_dc-p(
          v-if="item_comercialDocumentType && item_comercialDocumentType.id"
        )
          .it-pkey 
            label pkey
...
.l-dc-container
  .dc-0 
    ...
    FVPagination(
      v-bind:ldc="m_dc_comercialDocumentOut",
      v-on:setPage0="setPage0"
    ) 
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const load_comercialDocumentOut = (n) => {
      ...
      const comercialDocumentOutPayload = {
        dc: "comercialDocumentOut",
      ...
      doSave(payload)
        .then((r) => {
          if (r.id) {
            router.push({
              name: "indoc",
              params: { comercialType: "aa", docid: r.id },
            });
    ...
    const canAdd = () => {
      return (
        item_contract.value["id"] !== undefined &&
        item_comercialDocumentType.value["id"] !== undefined
      );
    };
    ...
    return {
      ...
      loadList_comercialDocumentType,
      ldco00_comercialDocumentType,
      item_comercialDocumentType,
      item_ac_comercialDocumentType,
      updateAC_comercialDocumentType,
      removeDC_comercialDocumentType,
      idxSelect_comercialDocumentType,select00
    };
  },
});
</script> 
